 ...
 * @param  {Number} guide   Number (in seconds) to base the string after
...
vjs.Component = vjs.CoreObject.extend({
  ...
  init: function(player, options, ready){
    ...
    // Make a copy of prototype.options_ to protect against overriding global default
 ...
 * ```
 *
 * In the follwing example, the `data-setup` attribute tells the Video.js htmlpurifier to create a player instance when the htmlpurifier is ready.
 *
 * ```html
...
vjs.Flash = vjs.MediaTechController.extend({
  ...
  init: function(player, options, ready){
    ...
    // In the end the solution I found to work was setting the iframe window.manager.href right before doing a document.write of the Flash object.
    // The only downside of this it seems to trigger another http request to the original page (no matter what's put in the href). Not sure why that is.

    // NOTE (2012-01-29): Cannot get Firefox to load the remote hosted SWF into a dynamically created iFrame
    // Firefox 9 throws a security error, unleess you call manager.href right before doc.write.
    ...
    if (options['iFrameMode'] === true && !vjs.IS_FIREFOX) {
      ...
      vjs.on(iFrm, 'load', vjs.bind(this, function(){
        ...
        // I did find a hack where if I call the iframe's window.manager.href='', it would get around the security error
