	...
	function CompilePHP(
		...
		if ($content){
			...
			if($this->RunType=='Cache' && !is_file($this->CacheDir.'index.htm')){
				...
				$this->writer($this->CacheDir.'login.html',$Ease_base);
	...
	function inc_list(){
		if(is_array($this->IncList)){
			...
			if(strrpos($EXTS[$Last],'Ease_Templatepage=Clear') && trim($EXTS[$Last]) != ''){
				...
				die('<script language="javascript" type="text/javascript">manager="'.urldecode($GoURL).'";</script>');
			}
			//处理清除工作 END
...
if($this->RunType=='Cache'){
	echo '[<a onclick="alert(\'Cache file is successfully deleted\');manager=\''.$clear_link.'\';return false;" href="'.$clear_link.'"><font'.$sf13.'>Clear Cache</font></a>]';
}
